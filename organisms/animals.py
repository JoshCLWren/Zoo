...
class Animal(Organism):
    ...
    def check_nearby_tiles(self):
        ...
        for row, col in itertools.product(range(-1, self.speed), range(-1, self.speed)):
            # check if the self.home_id.grid position exists
            if 0 <= self.position[0] + row < len(
                self.home_id.grid
            ) and 0 <= self.position[1] + col < len(self.home_id.grid[0]):
                # check if the self.home_id.grid position is occupied
                if (
                    self.home_id.grid[self.position[0] + row][self.position[1] + col]
                    is None
                ):
                    ...
                    self.animals_nearby.append(
                        self.home_id.grid[self.position[0] + row][
                            self.position[1] + col
                        ]
                    )
                else:
        ...
        for tile in self.nearby_unoccupied_tiles:
            is_safe = not any(
                (
                    0 <= tile[0] + row < len()
                    and 0 <= tile[1] + col < len(self.home_id.grid[0])
                )
                and (
                    self.home_id.grid[tile[0] + row][tile[1] + col] is not None
                    and isinstance(
                        self.home_id.grid[tile[0] + row][tile[1] + col], Animal
                    )
                )
    ...
    def drink(self, water):
        ...
        if water.size > 1:
            ...
            self.thirst -= 1
            self.energy -= 1
            self.home_id.remove_water(water)
            self.home_id.self.home_id.grid[water.position[0]][water.position[1]] = None
    ...
    def mate(self, partner):
        ...
        if partner.virility > 0 and self.age > 1 and partner.age > 1:
            ...
            for tile in self.nearby_unoccupied_tiles:
                row, col = tile
                if self.home_id.grid[row][col] is None:
                    baby = self.__class__()
                    baby.position = [row, col]
                    self.home_id.grid[row][col] = baby
                    return baby
    ...
    def die(self, reason):
        ...
        self.virility = 0
        self.is_alive = False
        # remove the animal from the self.home_id.grid
        self.home_id.self.home_id.grid[self.position[0]][self.position[1]] = None
        # remove the animal from the self.home_id
        with contextlib.suppress(ValueError):
            self.home_id.animals.remove(self)
        # replace the animal with a corpse if the position is not occupied
        if self.home_id.self.home_id.grid[self.position[0]][self.position[1]] is None:
            corpse = Corpse(self)
            self.home_id.self.home_id.grid[self.position[0]][self.position[1]] = corpse
        return reason
    ...
    def move(self, direction):
        ...
        """
        # Only move if the new position is within the self.home_id.grid and occupied by None, grass, or dirt
        moved = False
        ...
        if (
            ...
            if current_occupant:
                ...
                new_col = direction[1]
            if 0 <= new_row < len(
                self.home_id.self.home_id.grid
            ) and 0 <= new_col < len(self.home_id.self.home_id.grid[0]):
                # new position is within the self.home_id.grid, so update the position
                self.home_id.self.home_id.grid[self.position[0]][
                    self.position[1]
                ] = None
                self.position[0] = new_row
                self.position[1] = new_col
                self.home_id.self.home_id.grid[new_row][new_col] = self
                moved = True
        ...
        if moved:
            ...
            self.hunger -= 1
            self.thirst -= 1
            self.home_id.tiles_to_refresh.append(current_occupant)
    ...
    def motivation(self, turn_number):
        ...
        if not self.liveness_check():
            self.die(self.home_id, "natural causes")
    ...
    def liveness_check(self):
        ...
        self.look_for_water(self.home_id.grid, self.home_id)
    ...
    def turn(self, turn_number):
        ...
        self.motivation(turn_number, self.home_id)
        self.base_hunger(self.home_id.grid, self.home_id)
        self.base_thirst(self.home_id.grid, self.home_id)
        self.base_rest()
        if not self.home_id.full:
            self.base_reproduce(self.home_id.grid, self.home_id, turn_number)
        return self.motive

    def base_reproduce(self, turn_number):
        if partner := self.check_for_mating_partner():
            if baby := self.reproduce(partner, self.home_id, turn_number):
                self.home_id.animals.append(baby)

        else:
            # move towards random direction
            self.move([random.randint(-1, 1), random.randint(-1, 1)], self.home_id)
    ...
    def check_for_mating_partner(self):
        ...
        for i, j in itertools.product(range(-1, 2), range(-1, 2)):
            ...
            if (
                0 <= row < len()
                and 0 <= col < len(self.home_id.grid[0])
                and isinstance(self.home_id.grid[row][col], Animal)
                and self.home_id.grid[row][col].__str__() == self.__str__()
                and self.home_id.grid[row][col].gender != self.gender
                and self.home_id.grid[row][col].motive == "mate"
            ):
                return self.home_id.grid[row][col]
        return None

    def reproduce(self, partner, turn_number):
        # check if both animals have enough energy to reproduce
        self.home_id.check_full()
        self.check_nearby_tiles(self.home_id.self.home_id.grid)
        ...
        self.home_id.self.home_id.grid[baby_position[0]][baby_position[1]] = baby
        ...
        if self.motive == "sleep":
            if not self.is_at_safe_spot():
                if safe_spot := self.look_for_safe_spot(
                    self.self.home_id.self.home_id.grid
                ):
                    self.move(safe_spot)
    ...
    def base_thirst(
        ...
        if self.motive == "drink":
            if found_water := self.look_for_water(self.home_id.grid, self.home_id):
                self.move(found_water.position, self.home_id)
                self.drink(found_water, self.home_id)
            else:
                # move towards random direction
                ...
                if 0 <= new_pos[0] < len() and 0 <= new_pos[1] < len(
                    self.home_id.grid[0]
                ):
                    self.move(direction, self.home_id)
    ...
    def base_hunger(self, func=None):
        ...
        if self.motive == "eat":
            if found_food := func():
                self.move(found_food, self.home_id)
                self.eat(found_food, self.home_id)
            else:
                # move towards random direction
                self.move([random.randint(-1, 1), random.randint(-1, 1)], self.home_id)
    ...
    def look_for_food(self, limit=None):
        ...
        for i, j in itertools.product(
            ...
            if limit is not None and len(food) >= limit:
                break
            try:
                if self.home_id.grid[self.position[0] + i][
                    self.position[1] + j
                ] and isinstance(
                    self.home_id.grid[self.position[0] + i][self.position[1] + j],
                    self.favorite_food,
                ):
                    food.append(
                        self.home_id.grid[self.position[0] + i][self.position[1] + j]
    ...
    def look_for_water(self):
        ...
        """
        max_index_i = len(self.home_id.self.home_id.grid) - self.speed
        max_index_j = len(self.home_id.self.home_id.grid[0]) - self.speed
    ...
    def check_if_drowned(self, i_max, i_min, j_max, j_min):
        return all(
            isinstance(self.home_id.grid[i][j], Water)
            for i, j in itertools.product(range(i_min, i_max), range(j_min, j_max))
        ) and all(
            self.home_id.grid[i][j].size > self.size
            for i, j in itertools.product(range(i_min, i_max), range(j_min, j_max))
        )
    ...
    def look_for_safe_spot(self):
        ...
        # account for the edges of the self.home_id.grid
        if self.position[0] - self.speed < 0:
            self.position[0] = self.speed
        if self.position[0] + self.speed > len() - 1:
            self.position[0] = len() - 1 - self.speed
        if self.position[1] - self.speed < 0:
            self.position[1] = self.speed
        if self.position[1] + self.speed > len(self.home_id.grid[0]) - 1:
            self.position[1] = len(self.home_id.grid[0]) - 1 - self.speed
        ...
        while not has_eaten:
            for compatible_food_class in compatible_food_classes:
                if isinstance(found_food, (Plant, Corpse)):
                    ...
                    self.home_id.self.home_id.grid[found_food.position[0]][
                        found_food.position[1]
                    ] = None
                    found_food.die(self.home_id)
                    self.grow()
                    has_eaten = True
                    break
                elif isinstance(
                    ...
                    if attack > defense:
                        ...
                        if compatible_food_class.energy <= 0:
                            ...
                            compatible_food_class.die(self.home_id)
...
class Predator(Carnivore):
    ...
    def turn(self, turn_number):
        ...
        """
        self.motivation(turn_number, self.home_id)
        self.predator_hunger(self.home_id.grid, self.home_id)
        self.base_thirst(self.home_id.grid, self.home_id)
        self.base_rest()
        self.home_id.check_full()
        if not self.home_id.full:
...
class Scavenger(Animal):
    ...
    def turn(self, turn_number):
        ...
        """
        self.motivation(turn_number, self.home_id)
        self.base_hunger(self.home_id.grid, self.home_id, func=self.scavenge)
        self.base_thirst(self.home_id.grid, self.home_id)
        self.base_rest()
        self.home_id.check_full()
        if not self.home_id.full:
